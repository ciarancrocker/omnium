...
module.exports.isUserAuthenticated = async function(guildMember) {
  // Let's build a query!
  let query = 'SELECT true FROM PERMISSIONS WHERE ';

  // first the user ID
  query += `discord_id = '${guildMember.id}' `;

  // then loop over their roles
  for (let role of guildMember.roles.keyArray()) {
    query += `OR discord_id = '${role}' `;
  }
