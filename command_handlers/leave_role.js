...
module.exports = {
  ...
  handler: async function(message) {
    ...
    const userRoles = message.member.roles.array().filter((el) => el.name == roleName);
    if (userRoles.length == 0) {
      await messageHelpers.sendError(message, 'You are not a member of the specified role.');
      return;
    }
